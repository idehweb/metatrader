...
void OnTick() {
  ...
  {
    if (Buy_Stop(High[1], StopRange, newlot, SL, TP, MagicNo, TradeLog, Slip,
                 "Your description")) {
      //--succeed
...
bool Buy_Stop(double xhigh,   //-- your previous high
              ...
              double xlot,    //--lot size should be checked for minimum and
                              //maximum lot that allowed in trade
...
{
  ResetLastError();
  Print("* Opening Buy_Stop: ", xdesc, " at ", xrange, " pips above last high ",
        NormalizeDouble(xhigh, Digits));

  position =
      xhigh + (xrange * Dgt * Point);  //--price at xrange pips above xhigh
  target =
      position + (xtp * Dgt * Point);  //--target at xtp pips above position
  stop =
      position - (xsl * Dgt * Point);  //--stoploss at xsl pips below position

  ticket =
      OrderSend(Symbol(), OP_BUYSTOP, xlot, NormalizeDouble(position, Digits),
                xslippage, NormalizeDouble(stop, Digits),
                NormalizeDouble(target, Digits), xlog, xmagic, 0, clrBlue);

  if (ticket <= 0) {
    Print("! Buy_Stop failed with error #", GetLastError());
    return (false);
  } else {
    Print("* Position Buy_Stop ", xdesc, " .. ticket #", ticket,
          " placed successfully.");
    return (true);
  }
}  //--end of Buy_Stop
...
void CheckVolumeValue(double volume) {
  ...
  if (volume < min_volume) {
    if (alert_email == true)
      SendMail("Lot volume changed.",
               "Lot volume has changed to " + DoubleToString(min_volume, 2) +
                   " as minimum volume that allowed by broker.");
    if (alert_hp == true)
      SendNotification("Lot volume has changed to " +
                       DoubleToString(min_volume, 2) +
                       " as minimum volume that allowed by broker.");
    Print("Lot volume has changed to " + DoubleToString(min_volume, 2) +
          " as minimum volume that allowed by broker.");
    newlot = min_volume;
  } else
    newlot = Lot;
